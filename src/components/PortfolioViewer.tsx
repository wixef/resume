...
export default function PortfolioViewer({ items, index, onClose, onChange }: Props) {
  ...
  const viewer =
    activeItem ? (
      <AnimatePresence>
        <motion.div
          className="system-cursor fixed inset-0 z-[90] p-3 md:p-6"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        >
          ...
          <motion.div
            ...
            className="relative z-[1] flex h-full items-center justify-center overflow-hidden rounded-[32px] border border-white/10 bg-black shadow-[0_24px_80px_rgba(0,0,0,0.55)]"
          ...
          >
            <div className="relative h-full w-full">
              ...
              <motion.div
                ...
                className="pointer-events-none absolute inset-x-0 top-0 flex items-center justify-between p-4 md:p-5"
              >
                <div className="glass-pill pointer-events-auto rounded-full px-3 py-2 text-xs text-white/65 md:text-sm">
                  {activeItem.title}
                </div>
                <button
                  type="button"
                  onClick={onClose}
                  className="glass-pill pointer-events-auto rounded-full px-4 py-2.5 text-sm text-white/82 transition hover:bg-white/10"
                >
                  Закрыть
                </button>
              </motion.div>
              ...
              {!isPlaying ? (
                <button
                  type="button"
                  onClick={togglePlay}
                  className="glass-pill absolute left-1/2 top-1/2 z-[2] flex h-20 w-20 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full text-white shadow-[0_0_32px_rgba(0,243,255,0.16)] transition hover:scale-105"
                >
              ...
              <motion.div
                ...
                transition={{ duration: 0.2 }}
                className="absolute inset-x-0 bottom-0 z-[2] p-4 md:p-5"
              >
                <div className="glass-soft rounded-[26px] border border-white/10 px-4 py-4 shadow-[0_16px_40px_rgba(0,0,0,0.32)] backdrop-blur-xl">
                  <div className="mb-3">
                    <input
                      type="range"
                      min={0}
